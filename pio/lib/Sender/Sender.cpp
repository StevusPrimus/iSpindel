...
bool SenderClass::RTCSyncToNTP()
{
  ...
  CONSOLELN("");
  CONSOLE(F("Current time set to: "));
  CONSOLELN(asctime(&timeinfo));
  if (now > TIMECHECK)
  {
    return true;
  }
...
bool SenderClass::mqttConnect(const String &server, uint16_t port, const String &name, const String &username,
                              const String &password, const bool secure, const char CACert[], const char deviceCert[],
...
#if API_MQTT_HASSIO
bool SenderClass::enableHassioDiscovery(String server, uint16_t port, String username, String password, String name,
                                        String unit)
{
  ...
  if (response)
  {
    ...
    _mqttClient.publish((topicSensor + "gravity/config").c_str(),
                        ("{ \"uniq_id\": \"" + chipid +
                         ...
                         name + "/gravity\"," + device + "}")
                            .c_str(),
                        true);
    _mqttClient.publish((topicSensor + "interval/config").c_str(),
                        ("{ \"uniq_id\": \"" + chipid +
    ...
    _mqttClient.publish((topicNumber + "interval/config").c_str(),
                        ("{ \"uniq_id\": \"" + chipid +
                         "_interval\", \"name\": \"Set Update Interval\", \"unit_of_meas\": \"Seconds\", \"val_tpl\": \"{{ value_json "
                         "}}\", \"stat_t\": \"ispindel/" +
                         name + "/interval\"," + 
                         " \"cmd_t\" :\"ispindel/" +
...
bool SenderClass::sendBrewblox(String server, uint16_t port, String topic, String username, String password,
                               String name)
{
  bool response = mqttConnect(server, port, name, username, password);
  if (response)
